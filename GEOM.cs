...
namespace Xmods.DataLib
{
    public class GEOM
    {
        ...
        public void MatchUVsets(vertexForm[] vertexFormatToMatch)
        {
            ...
            for (int i = 0; i < this.vertexFormat.Length; i++)
            {
                ...
                if (this.vertexFormat[i].formatDataType == 3) numUVs++;
            }
            while (numUVs < uvInd)
            {
    ...
    /*    public void RemoveUVset(int UVsetIndex)
        ...
        {
            AutoSlotray(slotrayData, 0f, 0f, 0f);
        }

        public void AutoSlotray(Dictionary<uint, SlotRayData> slotrayData, float armAdjustmentX, float armAdjustmentY, float armAdjustmentZ)
        {
            ...
            foreach (KeyValuePair<uint, SlotRayData> slot in slotrayData)
            {
                ...
                if (distance < float.MaxValue)      // if the slot ray intersects at least one face of the mesh
                {
                    float test = slotAvgPos.Distance(origin);
                    Vector3 offset = distance <= test ? slotAvgPos - intersection : new Vector3();
                    if (slot.Key == 0x3BFF1EC1)
                    {
                        offset.X += armAdjustmentX;
                        offset.Y += armAdjustmentY;
                        offset.Z += armAdjustmentZ;
                    }
                    else if (slot.Key == 0x687E8BDB)
                    {
                        offset.X -= armAdjustmentX;
                        offset.Y += armAdjustmentY;
                        offset.Z += armAdjustmentZ;
